...
//        }
    ...
    fun applyRecursively(v: View, style: (View) -> Unit) {
        style(v)
        if (v is ViewGroup) {
            val maxIndex = v.getChildCount() - 1
            for (i in 0..maxIndex) {
    ...
    fun getContext(manager: ViewManager): Context = when (manager) {
        is ViewGroup -> manager.getContext()
...
//    }
    ...
    //    @JvmStatic
//    fun <T : View> initiateView(ctx: Context, clazz: (ctx: Context) -> T): T {
//    fun <T : View> initiateView(ctx: Context, viewClass: KClass<T>): T {
//        return clazz(ctx)
